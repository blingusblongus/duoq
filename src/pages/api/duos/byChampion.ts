...
export async function POST(context: APIContext) {
    ...
    switch (puuids.length) {
        case 2:
            sq = sql`SELECT s1.championName AS championName1, s2.championName AS championName2, 
                ...
                GROUP BY s1.championName, s2.championName
                ORDER BY smoothed_win_rate DESC`;
            break;
        case 3:
            sq = sql`SELECT
                s1.championName AS championName1,
                s2.championName AS championName2,
                s3.championName AS championName3,
                COUNT(*) AS games_played,
        ...
        case 4:
            sq = sql`SELECT
                ...
                s2.championName AS championName2,
                s3.championName AS championName3,
                s4.championName AS championName4,
                COUNT(*) AS games_played,
                COUNT(*) FILTER (WHERE s1.win = true) AS games_won,  -- Assuming all are on the same team, if s1 wins, all win
                COUNT(*) FILTER (WHERE s1.win = false) AS games_lost,
                COUNT(*) FILTER (WHERE s1.win = true) * ${LAPLACE_COEFFICIENT} / COUNT(*) AS win_rate,
                ...
                WHERE 
                    ...
                    AND ${Match.gameMode} = 'CLASSIC'
                GROUP BY championName1, championName2, championName3, championName4
                ORDER BY smoothed_win_rate DESC`;
            break;
        case 5:
            sq = sql`SELECT
                ...
                s3.championName AS championName3,
                s4.championName AS championName4,
                s5.championName AS championName5,
                COUNT(*) AS games_played,
                COUNT(*) FILTER (WHERE s1.win = true) AS games_won,  -- Assuming all are on the same team, if s1 wins, all win
                COUNT(*) FILTER (WHERE s1.win = false) AS games_lost,
                COUNT(*) FILTER (WHERE s1.win = true) * ${LAPLACE_COEFFICIENT} / COUNT(*) AS win_rate,
                ...
                WHERE 
                    ...
                    AND ${Match.gameMode} = 'CLASSIC'
                GROUP BY championName1, championName2, championName3, championName4, championName5
                ORDER BY smoothed_win_rate DESC`;
            break;
        default:
